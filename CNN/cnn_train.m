function [net, info] = cnn_train(imdb, expDir)
...
% 	run(fullfile(fileparts(mfilename('fullpath')), '../../', 'matlab', 'vl_setupnn.m')) ;
	...
	net = dagnn.DagNN() ;
	net.addLayer('conv1', dagnn.Conv('size', [7 7 1 60], 'hasBias', true, 'stride', [1, 1], 'pad', [0 0 0 0]), {'input'}, {'conv1'},  {'conv1f'  'conv1b'});
 	net.addLayer('pool1', dagnn.Pooling('method', 'max', 'poolSize', [4, 4], 'stride', [2 2], 'pad', [0 0 0 0]), {'conv1'}, {'pool1'}, {});
%  	net.addLayer('relu1', dagnn.ReLU(), {'conv1'}, {'relu1'}, {});


	net.addLayer('conv2', dagnn.Conv('size', [7 7 60 120], 'hasBias', true, 'stride', [1, 1], 'pad', [0 0 0 0]), {'pool1'}, {'conv2'},  {'conv2f'  'conv2b'});
 	net.addLayer('pool2', dagnn.Pooling('method', 'max', 'poolSize', [4, 4], 'stride', [2 2], 'pad', [0 0 0 0]), {'conv2'}, {'pool2'}, {});
% 	net.addLayer('relu2', dagnn.ReLU(), {'conv2'}, {'relu2'}, {});

%     net.addLayer('conv3', dagnn.Conv('size', [2 2 120 120], 'hasBias', true, 'stride', [1, 1], 'pad', [0 0 0 0]), {'relu2'}, {'conv3'},  {'conv3f'  'conv3b'});
...
% 	net.addLayer('relu3', dagnn.ReLU(), {'conv3'}, {'relu3'}, {});
	...
	net.addLayer('conv3', dagnn.Conv('size', [1 1 120 700], 'hasBias', true, 'stride', [1, 1], 'pad', [0 0 0 0]), {'pool2'}, {'conv3'},  {'conv4f'  'conv4b'});
	net.addLayer('relu1', dagnn.ReLU(), {'conv3'}, {'relu1'}, {});

	net.addLayer('classifier', dagnn.Conv('size', [1 1 700 10], 'hasBias', true, 'stride', [1, 1], 'pad', [0 0 0 0]), {'relu1'}, {'classifier'},  {'conv5f'  'conv5b'});
